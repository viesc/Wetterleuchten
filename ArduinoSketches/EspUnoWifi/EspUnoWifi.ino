...
const char default_router_ssid[] = "die basis"; //your router ssid
const char default_router_pswd[] = "chesini93pornbike"; //your router password

const char thisSSID[] = "Climate Justice Now";
const char thisPass[] = "ccad2019";

#define eeprom_size 4048
#pragma pack(push)
#pragma pack(1)
typedef struct
{
...
//#define debug(fmt, ...) if(msgclient)if(msgclient.connected())msgclient.printf(fmt, ##__VA_ARGS__)
#define debug(...)
...
const char *head_frm = "HTTP/1.1 %d OK\r\nContent-Type: text/html\r\nAccess-Control-Allow-Origin:*\r\nContent-Length: %d\r\nConnection : close\r\n\r\n";
...
void load_eeprom()
{
    ...
    {
        sprintf_P(econfig.apssid, thisSSID, ESP.getChipId());
        strcpy_P(econfig.appswd, thisPass);
    }
...
void netstat_html(WiFiClient wc)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        wifi_option += "<option value=\"" + WiFi.SSID(i) + "\">" + WiFi.SSID(i) + "</option>\r\n";
    }
    ...
    wc.printf_P(index_text, econfig.apssid, wifi_option.c_str(), econfig.ssid, localip.c_str(), subnet.c_str(), gateway.c_str(), dns.c_str());
}
...
void loop()
{
    ...
    if (msgclient && msgclient.connected())
    {
        ...
        {
            n = msgclient.readBytes(msgPipeBuff, n > sizeof(msgPipeBuff) ? sizeof(msgPipeBuff) : n);
    ...
    {
        n = Serial.readBytes(msgPipeBuff, n > sizeof(msgPipeBuff) ? sizeof(msgPipeBuff) : n);
        if (msgclient && msgclient.connected())
    ...
    if (avrOTA.hasClient())
    {
        ...
        int index;
        for (int i = 0; i < sizeof(avrOTA_Clients) / sizeof(avrOTA_Clients[0]); i++, avrOTA_Client_LastIndex++)
        {
            index = avrOTA_Client_LastIndex % (sizeof(avrOTA_Clients) / sizeof(avrOTA_Clients[0]));
            if (!avrOTA_Clients[index])
                break;
            if (!avrOTA_Clients[index].connected())
                break;
        }

        avrOTA_Client_LastIndex %= (sizeof(avrOTA_Clients) / sizeof(avrOTA_Clients[0]));
    ...
    for (int i = 1; i <= sizeof(avrOTA_Clients) / sizeof(avrOTA_Clients[0]); i++)
    {
        WiFiClient avrOTA_client = avrOTA_Clients[(avrOTA_Client_LastIndex + i) % (sizeof(avrOTA_Clients) / sizeof(avrOTA_Clients[0]))];
        if (avrOTA_client && avrOTA_client.connected() && avrOTA_client.available())
        {
            ...
            else if ((s.indexOf("index") > 0) || (s.indexOf(" / ") > 0) || (s.indexOf("serial") > 0))
            {
...
retry:
    ...
    {
        debug("stk500_loadaddr(): (a) protocol error, expect=0x%02x, resp=0x%02x\n", Resp_STK_INSYNC, buf[0]);
        return -1;
    }
...
static int stk500_paged_write(unsigned int addr, unsigned int n_bytes, unsigned char *data)
{
    unsigned char buf[16];
    ...
    retry:
        ...
        {
            debug("stk500_paged_write(): (a) protocol error, expect=0x%02x, resp=0x%02x\n", Resp_STK_INSYNC, buf[0]);
            return -4;
        }
...
static int stk500_paged_load(unsigned int addr, unsigned int n_bytes, unsigned char *data)
{
    unsigned char buf[16];
    int a_div;
    int tries;
    ...
    retry:
        ...
        {
            debug("stk500_paged_load(): (a) protocol error, "
                  "expect=0x%02x, resp=0x%02x\n",
                  Resp_STK_INSYNC, buf[0]);
            return -4;
        }

        if (stk500_recv(data, block_size) < 0)
            return -1;
        data += block_size;
        if (stk500_recv(buf, 1) < 0)
            return -1;
        ...
        if (buf[0] != Resp_STK_OK)
        {
            debug("stk500_paged_load(): (a) protocol error, expect=0x%02x, resp=0x%02x\n", Resp_STK_OK, buf[0]);
            return -5;
        }
    }
    return n_bytes;
...
int pageCheck(int curAddress)
{
    ...
    if (page != curpage)
    {
        if (dity)
        {
            ...
            int wb = stk500_paged_write(page * sizeof(write_page_buff), sizeof(write_page_buff), write_page_buff);
            if (wb != sizeof(write_page_buff))
                return -1;
            int rb = stk500_paged_load(page * sizeof(write_page_buff), sizeof(read_page_buff), read_page_buff);
            if (rb != sizeof(write_page_buff))
                return -1;
            int err = 0;
            for (unsigned int ck = 0; ck < sizeof(write_page_buff); ck++)
                if (read_page_buff[ck] != write_page_buff[ck])
                    err++;
            if (err)
                return -1;
        }
...
int sigleRecord(uint8_t *data, int len)
{
    ...
    case 0:
        for (int i = 0; i < recsize; i++)
        {
            if (pageCheck(baseaddr + addr + i))
                return -2;
            write_page_buff[(baseaddr + addr + i) % sizeof(write_page_buff)] = phexdata[i];
        }
        debug("\n");
        if (recsize)
            dity = 1;
        ...
        break;
    case 1:
        if (pageCheck(address + sizeof(write_page_buff)))
            return -1;
        break;
    case 2:
...
int handleHex(WiFiClient client, int hexlen)
{
    ...
    while (client.connected())
    {
        ...
        if (avrOTA.hasClient())
            break;
        int rl = client.readBytesUntil(':', lineBuff, min((int)sizeof(lineBuff) - 1, hexlen - totalLen - 1));
        if (rl < 10)
            return totalLen;
        if (rl & 1)
            return totalLen;
