...
#define DURATION_DAY 2465 // 9863 // ms duration for the data of one day
...
const float diffT[] = {4.9, 4.9, 3.4, 6.7, 6.9, 5.1, 7.0, 5.0, 6.4, 4.4, 5.1, 4.8, 1.9, 0.4, -1.4, 0.3, 3.4, 3.7};

void setup()
{
  Serial.begin(9600);
  Serial.print("Hi, y'all");
//
//  stripD1.begin();
//  stripD1.show();
 ...
 // stripN1.show();
  ...
  //test
  for(int i=0; i<NUMPIXELS_NEOPIX1; i++){
    stripN1.setPixelColor(i, stripN1.Color(0, 0, 150));
    stripN1.show();
    delay(1);
  }
  delay(300);
  for(int i=0; i<NUMPIXELS_NEOPIX1; i++){
    stripN1.setPixelColor(i, stripN1.Color(0, 0, 0));
    stripN1.show();
    delay(1);
  }
  delay(300);
...
//                   + GetDayFromIndex(currentDay) + String(".") + GetMonthFromIndex(currentDay) + String("."));
    ...
    float tempDiff = diffT[currentDay];
//    Serial.println(String("current temp diff: ") + tempDiff);
  ...
  curSat = (intensity == 0) ? 0 : (int)((abs(intensity) / (float)INTENSITY_MAX) * 55) + 200;
    ...
    switch (stripToUse) {
      case 0:
        whiteLEDs(NUMPIXELS_NEOPIX1, LEDFRINGE_NEOPIX1, intensity);
...
{
  const float normal = 4; // +/- this amount counts as "normal" temperature
