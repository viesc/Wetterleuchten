...
#define MIN_FLICKER_ON 3
#define MAX_FLICKER_ON 6
#define MIN_FLICKER_OFF 40
...
{    
  if (tDay >= DURATION_DAY || currentDay < 0)
  {
    ...
    Serial.print("current temp diff: ");
  ...
  curSat = (intensity == 0)? 0 : (int)((abs(intensity)/(float)INTENSITY_MAX)*175) + 80;

  UpdateLEDs(intensity, curHue, curSat);
...
void UpdateLEDs (short intensity, int curHue, short curSat)
{
  ...
  if (!isOn && random(0, map(intensity, 0, INTENSITY_MAX, MIN_FLICKER_ON, MAX_FLICKER_ON)) == 0)
  {
    isOn = true;
    value = maxValue;
    hue = curHue;
  ...
  for (byte i=0; i<NUMPIXELS; i+=LEDSTEP)
  {
    ...
    //float weirdShit2 = noise
    strip.setPixelColor(i, strip.ColorHSV(hue, saturation, max(0, (int)(value - (weirdShit1 * value / 80.0) - weirdShit2 * value / 40.0))));
  }
