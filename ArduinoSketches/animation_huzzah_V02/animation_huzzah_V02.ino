...
#define DATAPIN_STRIP1 14
#define DATAPIN_STRIP2 14
#define DATAPIN_STRIP3 14
#define CLOCKPIN 12
...
#define DURATION_DAY 5000 // ms duration for the data of one day
...
Adafruit_DotStar strip(NUMPIXELS, DATAPIN_STRIP1, CLOCKPIN, DOTSTAR_BGR);
...
{    
  ...
  */
  ...
  if (tDay >= DURATION_DAY)
  {
    ...
    Serial.println(String("current day: ") + currentDay);
  }
...
short GetIntensityFromPoti()
{
  ...
  short intensity = constrain((float)analogRead(A0)/650 * INTENSITY_MAX, 0, INTENSITY_MAX);
...
void UpdateLEDs (short intensity, int hue)
{
...
{
  const float normal = 1.4; // +/- this amount counts as "normal" temperature
  const float maxDiff = 15.0; // +/- this temperature counts as 100% difference (to our animation)
  
  if (abs(difference) < normal)
    return 0;

  return mapf2i(difference, normal, maxDiff, INTENSITY_MIN, INTENSITY_MAX);
}
...
{
  return meansHist[index] - means2018[index];
}
...
void PlayAnimation (short intensity)
{
  ...
  for (byte i=0; i<NUMPIXELS; i+=LEDSTEP)
  {
    value = map(intensity, INTENSITY_MIN, INTENSITY_MAX, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
  ...
  if (intensity > 0)
  {
    ...
    int fadeTime = 0;
    short value = map(intensity, INTENSITY_MIN, INTENSITY_MAX, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
